<script>
  ...
  const tooltip = ($session.user === undefined ? 'Login with Twitch' : 'View Profile')
  const openProfile = () => goto($session.user === undefined ? '/login' : '/profile');
</script>
    ...
    <div class="avatar">
      <div class="w-8 rounded-full">
        {#if $session.user !== undefined}
          <img src="{$session.user.profilePic}" class="object-cover w-full h-full" alt="avatar">
        {:else}
